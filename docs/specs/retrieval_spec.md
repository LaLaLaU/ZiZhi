...
从《资治通鉴》语料中优先定位对当前现实问题最相关的 `CaseProfile`，再补充 section/chunk 证据，为回答生成提供可解释依据。
...
正式版本采用“案例优先 + 白话优先 + 视角标签 + 史案拼装”的检索策略：
...
- 检索先命中 seed case 或 perspective profile
...
- 用 JSONL/本地表构建 case 索引与 chunk 索引
...
- 不对整段原文重复编码
- `actors.role` 可进入 `retrieval_text`，但必须使用现代可映射角色，不使用单纯古代官职
- 标签与其他元信息不并入主文本，而是单独存字段
